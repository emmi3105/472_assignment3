...
```{r get the US states shapefile, eval = FALSE}

# Shapefile with US states
us_states <- states()
```



```{r create the map plot, eval=FALSE}

# Set the tmap mode
tmap_mode("view")
  
# Plot the regions
my_plot <- tm_shape(us_states) +
  tm_polygons("REGION") +
  
  # Dots for non-Ivy League universities
  tm_shape(geography_sf[geography_sf$Ivy_League == 0, ]) +
  tm_dots(size = 0.2, col = "Control", title = "University Status") +
  
  # Symbols for Ivy League universities
  tm_shape(geography_sf[geography_sf$Ivy_League == 1, ]) +
  tm_dots(size = 0.5, col = "#FF0000", shape = 18, title = "Ivy League") +
  
  tm_layout(title = "US Regions with Universities")
...
tm_shape(geography_sf) +
  tm_dots(size = 0.2, 
          col = ifelse(geography_sf$Ivy_League == 1, "red", 
                        ifelse(geography_sf$Control == "Public", "blue", "green")),
          title = "University Type") +
  
  # Plot Ivy League universities
  tm_shape(geography_sf[geography_sf$Ivy_League == 1, ]) +
  tm_dots(size = 0.5, shape = 18, col = "red", title = "Ivy League") +
  
  # Layout settings
  tm_layout(title = "US Universities")
...
tmap_save(my_plot, filename = "geographic_plot.png")

## save as stand-alone HTML file ("view" mode)
tmap_save(my_plot, filename = "geographic_plot.html")
```

#```{r}
knitr::include_graphics("geographic_plot.png")

#```

```{r include_map, echo=FALSE}

htmltools::includeHTML("geographic_plot.html")
```
