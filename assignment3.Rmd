...
# Call check_table on "universities"
...
# name <- get_university_name(231352685)
```
...
```{r call the get_finance_data() function, echo=TRUE}

# Call the "get_finance_data()" function
result_list <- lapply(ivyleagues$ein, get_finance_data)
finance_df <- do.call(rbind, result_list)
```

#### Format the finance_df

As previously mentioned, the finance_df that we retrieved needs to be modified before writing it to the database. Not for all universities the data on the revenue and assets is available for each year from 2010 to 2020 (inclusive). Secondly, due to the nature of our function and the structure of the API's data, we have also extracted data for years that we are not interested in. We will delete these rows and create rows with missing values for observations of years of interest that are missing.

```{r format the finance_df}

# Filter for observations of the years 2010-2020
finance_df <- finance_df %>%
  filter(year >= 2010 & year <= 2020) %>%
  na.omit() 

# Fill NA values for missing years
complete_df <- expand.grid(year = 2010:2020, 
...
**Note**: The final dataframe has missing values. For instance, the data for total assets and total revenue of Harvard University is unavailable for the years 2010 and 2020.
...
# Testing that it worked with a simple query
#dbGetQuery(db, "SELECT * FROM finance_df LIMIT 18")
...
```{r retrieve median estimated household income}
...
census_df <- census_df %>%
  ...
  # For 2015
  left_join(select(median_household_income_15, NAME, estimate), by = c("County" = "NAME")) %>%
  mutate("2015" = estimate) %>%
  select(-estimate)  %>%
  
  # For 2020
  left_join(select(median_household_income_20, NAME, estimate), by = c("County" = "NAME")) %>%
