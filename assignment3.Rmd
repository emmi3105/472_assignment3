...
# Create database
db <- dbConnect(RSQLite::SQLite(), "data/universities_db.sqlite")
```
...
print(file.exists("data/universities_db.sqlite"))
```
...
We combined exercise 2a and 2b and directly wrote and executed a webscraping function called `scrape_wikipedia_tables()` that returns a data frame with the following eight variables: 
...
save(scraped_wikipedia_df, file = "data/scraped_wikipedia_df.RData")
```

```{r create a copy of scraped_wikipedia_universities}

# Load global variables
load("data/scraped_wikipedia_df.RData")
...
ivyleagues <- read.csv("data/ivyleague.csv")
```
...
iv. `ein`: the Employer Identification Number (EIN)

The primary key in the relational database that we will later add our tables to is the `Institution` variable, which refers to the full name of the university. Hence, we created a new variable in the `ivyleagues` data frame that gives the full name instead of the shortened version for each Ivy League. We store this variable under the column name `Institution`. This ensures that we can later properly query the relational database. 
...
if (ncol(universities_table) == 11) {
  ...
  column_answer <- sprintf("The dataframe has %d columns, which each represents one variable:", ncol(universities_table))
...
# Save the global variable to an RData file
save(scraped_ivyleagues_df, file = "data/scraped_ivyleagues_df.RData")
```
...
# Load global variables
load("data/scraped_ivyleagues_df.RData")
...
```{r check the existence of ivyleagues_ranks_df, echo=TRUE}

# Check the existence of the ivyleagues_ranks_df table
...
# A function that filters for the social sciences in the ranking options
...
# Save the global variable to an RData file
save(social_science_ranks, file = "data/social_science_ranks.RData")
```
...
# Load global variables
load("data/social_science_ranks.RData")
...
# Check the existence of the social_science_ranks_df table
...
# Save the global variable to an RData file
save(scraped_finance_df, file = "data/scraped_finance_df.RData")
```
...
# Load global variables
load("data/scraped_finance_df.RData")
...
# Check the existence of the finance_df table
...
We use the `tidycensus` package to access the API of the US Census Bureau. We write a function that retrieves the names of all the counties in the US and the estimated median household income for every county for both 2015 and 2020 (based on the American Community Survey (ACS)). 
...
# Save the global variable to an RData file
save(median_household_income_15, file = "data/median_household_income_15.RData")
save(median_household_income_20, file = "data/median_household_income_20.RData")
```
...
# Load global variables
load("data/median_household_income_15.RData")
load("data/median_household_income_20.RData")
...
**a. Logarithmic scale**: Note that we have used a logarithmic scale for the x and y axes. The reason for that lies in the data, which is mostly concentrated in a small range of values, yet has some outliers (particularly Dartmouth College). When dealing with skewed data or data with wide data ranges, it can be helpful to use logarithmic scales for the axes. A linear scale may compress the data at one end and stretch it at the other, making it difficult to visualise patterns. A logarithmic scale ensures that data points across the entire range are more evenly distributed. If the data is skewed, it can help in visualising details in the tail of the distribution that might be compressed in a linear scale.
...
There seems to be a slight positive correlation between the average median household income for the county and the endowment per student, which is not surprising. A rich county might facilitate an attractive environment for wealthy institutions of higher education or even positively contribute to a university's financial means. Potential explanations for that could be:
...
ii. where the colour of the points varies by status:
    - Private (non-profit) universities: <span style="color: #FFC20A;">**orange**</span> point.
    - Public universities: <span style="color: #F781BF;">**pink**</span> point.
...
As seen in the map above, the Ivy League universities are all concentrated in the East Coast of the United States, particularly in New England and the Mid-Atlantic region. Potential explanations for this phenomenon could be:
...
Public and private universities are seemingly spread across the country in equal patterns. However, when displaying the map in the mode `OpenStreetMap` instead of the default mode `Esri.WorldGrayCanvas`, and zooming in, we can notice that most private universities are located in close proximity to larger cities, such as Los Angeles, Dallas, Chicago, Washington DC, New York City, and Boston.
