...
```{r call the global rank scraping function, echo=TRUE}

# Call the global rank scraping function
ivyleagues_ranks <- scrape_university_ranking_global()
```
...
```{r calculate the midpoint for social science ranks}
...
```{r check the existence of the social_science_ranks table, echo=TRUE}
...
Lastly, we navigate back to the homepage.

QUESTION: maybe we can already close the driver here no?
...
##### 1. Function that gets the asset amount for a given ein

We create a function that will return the asset_amount for the universities given the ein number we put in.

```{r create get_asset_amount_function}

# Function to get asset amount for a given ein
get_asset_amount <- function(ein) {
  url <- paste0("https://projects.propublica.org/nonprofits/api/v2/organizations/", ein, ".json")
  response <- GET(url)
  
  if (!http_error(response)) {
    data <- content(response, "parsed")
    asset_amount <- data$organization$asset_amount
    return(asset_amount)
  } else {
    warning(paste("Failed to retrieve data for eid:", ein))
    return(NA)
  }
}
```

##### 2. Function that gets the revenue amount for a given ein

Furthermore, we create a GET request that will return the revenue_amount for the universities given the ein number we put in.

```{r create get_revenue_amount function}

# Function to get revenue amount for a given ein
get_revenue_amount <- function(ein) {
  url <- paste0("https://projects.propublica.org/nonprofits/api/v2/organizations/", ein, ".json")
  response <- GET(url)
  
  if (!http_error(response)) {
    data <- content(response, "parsed")
    revenue_amount <- data$organization$revenue_amount
...
We also need a dataframe that we can then add the extracted asset and revenue values to. The data frame "finance_df" has eight rows, which all represent the ivyleagues as in the "ivyleagues" dataframe. It has four variables which are represented by one column each. 
...
# Create a data frame for the financial information
...
Now, we will run the two functions, get_asset_amount and get_revenue_amount, to extract the total assets and the total revenue for each ivyleague university. We use sapply() to apply the function to each observation in finance_df.

```{r extract total_assets and total_revenue, echo=TRUE}

# Update total_assets column using the get_asset_amount function
finance_df$total_assets <- sapply(finance_df$ein, get_asset_amount)

# Update total_revenue column using the get_revenue_amount function
finance_df$total_revenue <- sapply(finance_df$ein, get_revenue_amount)
...
# Problem: Harvard has the value 0 for the total_revenue and the total_assets
ein_harvard = 42103580
url_harvard <- paste0("https://projects.propublica.org/nonprofits/api/v2/organizations/", ein_harvard, ".json")
response_harvard <- GET(url_harvard)
data_harvard <- content(response_harvard, "parsed")
```


```{r}
## data_harvard$filings_with_data[[1]] corresponds to year 2021
data_harvard$filings_with_data[[1]]$tax_prd_yr

## therefore, we can get the total revenue through this code:
data_harvard$filings_with_data[[1]]$totrevenue

## and the total assets
data_harvard$filings_with_data[[1]]$totassetsend



## Try another year:
data_harvard$filings_with_data[[2]]$tax_prd_yr
...
# Testing that it worked with a simple query
#dbGetQuery(db, "SELECT * FROM finance_df LIMIT 8")

```


We can test whether the scraping worked properly by using the “check_table()” function. It should return a table of XXXX rows and XX columns, namely "XXX", "XXX", and "XXX".
